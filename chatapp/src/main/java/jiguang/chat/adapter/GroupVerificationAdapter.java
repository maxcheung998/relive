    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        JMessageClient.getGroupInfo(Long.parseLong(entry.groupName), new GetGroupInfoCallback() {
            ...
            public void gotResult(int i, String s, GroupInfo groupInfo) {
                holder.item_reason.setText("申請加入群 " + groupInfo.getGroupName());
                //邀请
                if (entry.groupType == 0) {
                    holder.item_name.setText(entry.toDisplayName);
                    holder.tv_groupInvite.setText("邀請人：" + entry.fromDisplayName);//事件发起人
                } else {
                    holder.item_name.setText(entry.toDisplayName);
        ...
        if (entry.btnState == 0) {
                    ...
                    event.acceptGroupApproval(entry.toUsername, entry.appKey, new BasicCallback() {
                        ...
                        public void gotResult(int i, String s) {
                            if (i == 0) {
                                ...
                                Toast.makeText(mContext, "添加失敗" + s + i, Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
            ...
            event.refuseGroupApproval(entry.toUsername, entry.appKey, "拒絕加入", new BasicCallback() {
                @Override
                public void gotResult(int i, String s) {
                    if (i == 0) {
                        Toast.makeText(mContext, "拒絕成功", Toast.LENGTH_SHORT).show();
                        holder.item_add_btn.setBackgroundColor(Color.parseColor("#FFFFFF"));
                        holder.item_add_btn.setTextColor(Color.parseColor("#B5B5B6"));
                        holder.item_add_btn.setText("已拒絕");
                        entry.btnState = 2;
                        entry.save();
                    } else if (i == 856002) {
                        holder.item_add_btn.setBackgroundColor(Color.parseColor("#FFFFFF"));
                        ...
                        Toast.makeText(mContext, "拒絕失敗" + s + i, Toast.LENGTH_SHORT).show();
                    }
                }
            });
